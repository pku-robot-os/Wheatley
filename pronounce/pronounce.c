...
int play(const char *name)
{
	...
	double ts = wav_header.wSampleLength * 8.0 / (wav_header.nSamplesPersec * wav_header.wBitsPerSample);
	printf("预计播放时间:%.2lfs\n", ts);


	signal(SIGALRM, SIG_DFL);
	alarm((int)(ts * 1.2 + 1.0));
	set_pcm_play(fp);
	alarm(0);
...
int main()
{
	...
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5a61f069, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
